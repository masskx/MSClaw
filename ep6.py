...
ASSISTANT_NAME = os.getenv("ASSISTANT_NAME", "Memory Bot")
...
def ensure_workspace()->None:
    """创建工作目录，conversation目录和CLAUDE.md文件，如果不存在的话"""
...
async def run_agent(prompt:str,bot:Any,chat_id:int)->str:
    ...
    async with _agent_lock:
        return await _run_agent_inner(prompt,bot,chat_id)
...
async def _run_agent_inner(prompt:str,bot:Any,chat_id:int)->str: # 调用模型
    env = {
...
async def handle_message(update:Update,context):
    ...
    if not update.message or not update.message.text: # 如果用户的消息为空
        return
    response = await run_agent(prompt=update.message.text,bot=context.bot,chat_id=update.effective_chat.id)
    # 归档对话，用于长期记忆
    archive_conversation(user_text, response)
...
async def clear(update:Update,context)->None:
    ...
    if not is_owner(update):
        await update.message.reply_text("你没有权限使用此Bot")
        return
    await update.message.reply_text("会话已清除，请重新开始")
...
def main():
    ...
    #app注册器
    # add_handler 添加处理程序
    app.add_handler(CommandHandler("clear",clear)) # 处理/end命令
    ...
    print(f"{ASSISTANT_NAME} 已启动！")
    print(f"会话状态：{'已恢复' if session_id else '新会话'}")
    print(f"工作目录：{WORKSPACE_DIR}")
    print(f"长期记忆：{WORKSPACE_DIR / 'CLAUDE.md'}")
    print(f"对话归档：{CONVERSATIONS_DIR}")
    app.run_polling()
